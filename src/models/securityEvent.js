...
async function create(websiteId, eventTypeId, sourceIp, eventDatetime, continentCode, countryCode, details = null) {
  ...
  const result = await db.query(sql, [websiteId, eventTypeId, sourceIp, eventDatetime, continentCode, countryCode, detailsJson]);
...
async function bulkCreate(events) {
  ...
  if (events.length === 1) {
    ...
    try {
      await create(event.websiteId, event.eventTypeId, event.sourceIp, event.eventDatetime, event.continentCode, event.countryCode, event.details);
