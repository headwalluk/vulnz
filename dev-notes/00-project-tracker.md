...
**M17 — agent query access: the API side is done.** v1.39.2 is live on prod (verified). The wordpress.org backfill completed 2026-08-16: **7,409 closed plugin components** catalogued, 7 left unresolved on transient errors. On the fleet that is 23 security-withdrawn plugins on 28 production sites, plus 10 unclassified-reason closures on a further 30 — the unclassified bucket affects _more_ production sites than the confirmed-security one, which is the argument for the tri-state field in one line. Remaining M17 items are the access model (M17.4), the `fields=` selector (M17.9) and the doc guard (M17.11).
...
- [x] **M17.6** — **Partly done in v1.39.2:** `limit` is capped by `API_MAX_PAGE_SIZE` (default 200) on both list endpoints, and page/limit are validated rather than silently defaulted. The **summary mode is still outstanding** and is the more valuable half — see M17.9.

- [x] **M17.10** — **Agent review of the query surface, done in v1.39.2.** Paul's agent read `docs/agent-queries.md` and reviewed the endpoints behind it — the first consumer to exercise M17 end to end. Findings, all now fixed: `component_version`/`component_type` silently ignored without `component_slug` (returned the whole fleet, reading as "every site runs this version"); `component_type` unvalidated so a typo returned "nothing affected"; `limit` uncapped and unvalidated with the docs claiming a max that did not exist; `GET /api/components` returning raw column names while the single-component routes returned friendly ones; and `process.env.LIST_PAGE_SIZE` read directly against the project's own rule.
  - The pattern worth remembering: **every one of these was an input the API accepted and quietly ignored.** The deliberate `400` on an unknown `sort` was the right instinct and simply had not been applied consistently. When adding a filter, decide what an unusable combination does _before_ shipping it — the default of ignoring it produces a confident wrong answer, which is the worst outcome for a security tool
  - It also flagged the biggest gap correctly: the closure data was unreachable without already knowing a slug. That is now `?wporg_status=` on components and `?component_wporg_status=` on websites
- [ ] **M17.9** — **`fields=` selector or summary mode on `GET /api/websites`.** The remaining half of M17.6, and the agent's second request. The response embeds every plugin and theme per site (~5.9 KB each), so a blast-radius query returns the full inventory of every matching site when the caller wants one component. Roughly 95% of that payload is waste for the common case. Not blocking — server-side filters now narrow most questions to a handful of sites — but it is the lever on the 450k-token fleet-wide figure, and it matters more than the `limit` cap did

- [ ] **M17.11** — **Keep `docs/agent-queries.md` accurate mechanically.** Paul's standard (2026-08-16): it may be incomplete, but it must never be inaccurate — it is the doc his agent actually works from, and it outranks `api-usage.md` for that purpose. An incomplete doc costs a capability the agent does not use; an inaccurate one costs a false assertion about the fleet's security posture that nobody catches. Proposed guard: a test that parses `GET /api/...?param=` out of the doc and asserts each path and parameter exists in the generated OpenAPI spec. Catches renames and removals; will not catch semantic drift, so the discipline of running every example before writing it down still stands. That discipline already caught the `?q=` "domain or title" error the spec had carried for a long time
