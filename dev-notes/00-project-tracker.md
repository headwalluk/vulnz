...
- [ ] **M17.6** — Cap `limit` on `GET /api/websites`, or add a summary mode. **Measured against prod 2026-08-16, and the original framing was wrong:** the N+1 is not the problem. `limit=1` → 247 ms, `limit=10` → 255 ms, `limit=50` → 327 ms, so the three-queries-per-site cost is nearly flat against an indexed database and extrapolates to roughly a second for all 307 sites. The real cost is **payload size**: ~5.9 KB per site, so a whole-fleet pull is ~1.8 MB. For a browser that is nothing; for the agent (M17.8) it is on the order of 450k tokens of JSON to answer a question that needs three fields. A `fields=` selector or a summary mode is worth considerably more than a `limit` cap, and the cap alone would just make the agent paginate to the same total
